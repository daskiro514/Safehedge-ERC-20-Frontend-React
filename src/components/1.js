...
const Dashboard = (props) => {
	...
	const handleMint = async (e) => {
		...
		const contract = new web3.eth.Contract(TokenABI, "0x9de02c024916c3636d34f26fd45cb0b16004c2a2");

		const numberOfMint = mintValue + "000000000000000000";

		console.log(numberOfMint)

		const from = localStorage.getItem('walletAdr') ? localStorage.getItem('walletAdr') : '0x8fCe3B2B7B052C5858dB41e5606C2F2c2A08c478';

		contract.methods._mint(numberOfMint).send({from: from})
	...
	const handleBurn = e => {
		...
		contract.methods._burn(numberOfBurn).send({from: from})
			.on('transactionHash', function(hash){
			    console.log(hash);
			})
			.on('receipt', function(receipt){
